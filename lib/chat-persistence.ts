...
/**
 * Cuantas conversaciones ESPERAN RESPUESTA, y de cuando es la mas nueva.
 *
 * Es la mitad de chats del numero de la pestaña, y existe por una razon
 * concreta: la otra forma de tenerlo —`useChatUnreadStore`— la escribe **solo
 * la bandeja**, asi que en cualquier pantalla que no sea Chats arranca en cero
 * y se queda ahi. Desde fuera eso no se ve como un numero que falta: se ve como
 * un contador que no funciona, porque el icono no cambia nunca.
 *
 * # Por que esta definicion y no «lo no leido»
 *
 * Porque **lo no leido de un WhatsApp no vive en nuestra base**. Se comprobo
 * antes de escribir esto:
 *
 * - `persistedRowToChat` pone `unreadCount` a 1 solo en los canales del store
 *   unificado (Telegram y Meta); para WhatsApp escribe **0 siempre**.
 * - El «no leido» de la bandeja sale de juntar el `unreadCount` de Evolution
 *   con las marcas de `seenMessages`, que son del **navegador** (`localStorage`,
 *   `hooks/chats/useSeenMessages`). Ni una de las dos esta guardada aqui.
 *
 * Preguntarle a Evolution desde aqui queda descartado por la regla de siempre:
 * esto lo pide un reloj que corre en TODAS las pantallas de todo el mundo, y
 * `fetchChatsFromEvolution` es el camino caro — ademas de no existir para Waha
 * ni para los canales de credenciales. Era justamente lo que hacia la accion
 * muerta que esto sustituye.
 *
 * Lo que si es cierto, duradero y vale para los tres proveedores es la FORMA
 * del ultimo mensaje: **si el ultimo lo escribio el contacto, esa conversacion
 * espera respuesta**. Es exactamente el criterio con el que este mismo fichero
 * ya calcula el `unreadCount` de Telegram y Meta (`row.fromMe === false`), y es
 * el criterio del propio numero de la pestaña —«solo lo que exige respuesta»—.
 *
 * # Y `masNuevo` no es decoracion
 *
 * La bandeja sabe MAS que esto —conoce las marcas de leido de este navegador—
 * asi que cuando ha hablado manda ella. Lo unico que puede desmentirla es que
 * haya entrado algo **mas nuevo de lo que ella llego a juzgar**, y para saberlo
 * hace falta la hora del mas reciente, no el contador. Quien decide es
 * `elNumeroDeChats` (`lib/insignia-del-favicon.ts`), que es puro.
 */
export async function contarChatsSinLeer(params: {
  userIds: string[];
  /**
   * Las lineas de la bandeja. **Sin esto el numero cuenta de mas**: en
   * `chat_conversations` quedan las lineas borradas, los restos `_V2` y los
   * canales `_wh`, y ninguna de esas sale en la lista. Es el mismo fallo que
   * ya costo una vuelta en `refetchChatsManualAction`.
   */
  instanceNames: string[];
  ...
  const userIds = params.userIds.filter(Boolean);
  const lineas = params.instanceNames.filter(Boolean);
  if (!userIds.length || !lineas.length) return vacio;

  try {
    const __t0 = performance.now();
    const filas = await db.$queryRaw<{ total: bigint; masNuevo: Date | null }[]>`
      WITH marcas AS (
        SELECT DISTINCT "userId", "remoteJid"
        FROM "ChatConversationPreference"
        WHERE "userId" IN (${Prisma.join(userIds)})
      ...
      SELECT
        -- La MISMA conversacion puede tener dos filas: una entrada por su
        -- @lid y otra por su numero. La bandeja las junta al pintar, asi que
        -- contando filas el icono diria dos donde hay una. senderPn es el
        -- numero real de la fila que entro por @lid, asi que las dos caen en
        -- la misma llave. Y con la linea dentro, porque el mismo contacto
        -- escribiendo a dos lineas SI son dos conversaciones.
        -- (Sin acentos graves aqui dentro: cierran el template literal.)
        COUNT(DISTINCT (
          c."instanceName",
      ...
      WHERE c."userId" IN (${Prisma.join(userIds)})
        AND c."instanceName" IN (${Prisma.join(lineas)})
    ...
    // Un contador nunca deberia costar; si algun dia cuesta, que se vea. Este
    // ademas lo pide el reloj que corre en todas las pantallas.
    const __ms = performance.now() - __t0;
    if (__ms > 300) {
      console.warn(`[PERF] contarChatsSinLeer ${Math.round(__ms)}ms`, {
        cuentas: userIds.length,
    ...
    return {
      ...
      masNuevo: f.masNuevo ? f.masNuevo.getTime() : 0,
    };
  } catch (error) {
    // Mudo aqui se ve como que el icono no cuenta los chats, que es el fallo
    // del que viene todo esto.
    console.error("[chats] no se pudo contar lo que espera respuesta", error);
    return vacio;
  }
}
