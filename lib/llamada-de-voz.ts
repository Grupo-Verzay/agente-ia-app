...
/**
 * Cuánto se deja siempre visible por cada borde, al arrastrar la tarjeta.
 *
 * No es decoración: la tarjeta lleva dentro el botón de colgar. Dejarla salir
...
/**
 * Dónde puede quedarse la tarjeta de llamada tras arrastrarla.
 *
 * Es puro a propósito: el arrastre en sí es pointer events y no se prueba sin
 * navegador, pero **dónde acaba** sí, y es lo único que puede dejar la llamada
 * inalcanzable.
 *
 * Y el suelo de cada eje es el margen, **no** el máximo: en una ventana más
 * estrecha que la tarjeta —un móvil con la tarjeta expandida— el máximo sale
