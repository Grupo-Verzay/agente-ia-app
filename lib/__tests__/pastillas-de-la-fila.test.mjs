...
/** Lo que baja el relleno de cada pastilla, por lado. */
const MENOS = 2;

/**
 * Las seis pastillas del caso, en su orden, con su relleno horizontal de ANTES
 * (por lado), su alto y su tamaño de letra. El modo roto comprueba esta tabla
 * contra el código de antes; el bueno, que solo el relleno cambió.
 */
const PASTILLAS = [
    { que: "estado", relleno: 8, alto: 24, letra: "12px" },
    { que: "asignar", relleno: 4, alto: 24, letra: "10px" },
    { que: "recordatorios", relleno: 6, alto: 24, letra: "10px" },
    { que: "flujos", relleno: 8, alto: 24, letra: "12px" },
    { que: "seguimientos", relleno: 8, alto: 24, letra: "12px" },
    { que: "etiquetas", relleno: 8, alto: 24, letra: "12px" },
];
...
function comprobarLasPastillas(f, detalle) {
    ...
    PASTILLAS.forEach((esperada, i) => {
        ...
        const relleno = ROTO ? esperada.relleno : esperada.relleno - MENOS;
        assert.deepEqual(p.relleno, [relleno, relleno], `${esperada.que}: relleno por lado (${detalle})`);
        assert.equal(Math.round(p.alto), esperada.alto, `${esperada.que}: el alto no cambia`);
        assert.equal(p.letra, esperada.letra, `${esperada.que}: el tamaño de letra no cambia`);
    });
}
