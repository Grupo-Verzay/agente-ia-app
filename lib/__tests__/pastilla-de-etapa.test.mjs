 ...
 *   2. Está entre la de estado y «Asignar», medido en píxeles y no por el
 *      orden del JSX.
...
function medir() {
    ...
    for (const raiz of document.querySelectorAll("[data-fila]")) {
        ...
        const caja = etapa?.parentElement ?? estado?.closest("div") ?? raiz;
        filas[id] = {
            estado: de(estado),
            etapa: de(etapa),
            asignar: de(asignar),
...
test("va detrás del estado y delante de «Asignar»", () => {
    if (ROTO) return;
    for (const ancho of ANCHURAS) {
        for (const id of ["corta", "larga", "anchas"]) {
            const { estado, etapa, asignar } = POR_ANCHURA[ancho].filas[id];
            assert.ok(asignar, `falta «Asignar» en ${id}`);
            assert.ok(estado.r <= etapa.l, `la etapa se monta sobre el estado en ${id} a ${ancho}`);
            assert.ok(etapa.r <= asignar.l, `la etapa va después de «Asignar» en ${id} a ${ancho}`);
        }
    }
});
