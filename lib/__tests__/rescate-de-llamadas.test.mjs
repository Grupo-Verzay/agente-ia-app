...
/* ── G. El middleware: la puerta por la que se caía todo ────────────────── */

/**
 * Lo que este barrido caza, y que ningún banco de esta familia cazaba:
 * **una ruta que el backend llama y que el middleware manda al login**.
 *
 * El aviso de fin de llamada (#877) estaba bien escrito y su banco verde — y
 * en producción **no funcionó nunca**, porque su banco llama al manejador
 * directamente y ahí no hay middleware. Medido sobre el build servido:
 *
 *     POST /api/calls/call-ended  ->  307  ->  /login?callbackUrl=...
 *     y como `fetch` SIGUE las redirecciones: resp.ok true, status 200
 *
 * O sea que el backend escribía «fin de llamada avisado a la App» habiendo
 * entregado nada. Un fallo que se anuncia como un éxito es el más caro de
 * todos, y por eso esto se comprueba como un BARRIDO y no caso a caso: lo que
 * hay que impedir es que la PRÓXIMA ruta para el backend nazca con el mismo
 * agujero.
 */
test("G1 · toda ruta que el BACKEND llama pasa el middleware sin sesión", async () => {
    const { readFileSync, readdirSync, statSync } = await import("node:fs");
    const { join } = await import("node:path");

    // Los prefijos se LEEN del middleware, no se copian: una lista copiada es
    // una que el día que se añada un prefijo se queda corta.
    const src = readFileSync("middleware.ts", "utf8");
    const prefijos = [...src.matchAll(/^const api\w*Prefix = "([^"]+)";/gm)].map((m) => m[1]);
    const pasan = [...src.matchAll(/startsWith\((api\w*Prefix)\)\)\s*return NextResponse\.next\(\)/g)].map((m) => m[1]);
    assert.ok(prefijos.length >= 8, `se leyeron ${prefijos.length} prefijos del middleware`);
    // No basta con declarar el prefijo: hay que USARLO en un `return next()`.
    assert.equal(pasan.length, prefijos.length, "hay un prefijo declarado que nadie deja pasar");

    // Las rutas para el backend son las que se autentican con la clave
    // interna: nadie con un navegador la tiene, así que su puerta no es la
    // sesión.
    const rutas = [];
    (function recorrer(dir) {
        for (const e of readdirSync(dir)) {
            ...
            else if (e === "route.ts") rutas.push(f);
        }
    })("app/api");

    const delBackend = rutas.filter((f) =>
        /CRM_FOLLOW_UP_RUNNER_KEY|BILLING_CRON_RUNNER_KEY/.test(readFileSync(f, "utf8")),
    );
    assert.ok(delBackend.length >= 3, `se encontraron ${delBackend.length} rutas para el backend`);

    // Lo que se queda fuera va con su MOTIVO escrito al lado, y el banco falla
    // si el motivo está vacío: una exclusión sin explicación es una ruta rota
    // que alguien dio por revisada.
    const seQuedanFuera = {
        "app/api/bookings/appointment/route.ts":
            "importa ficheros `'use server'` y abrir su prefijo pone en rojo acciones-de-sistema: frente aparte, SIGUE ROTA",
        "app/api/bookings/slots/route.ts":
            "importa `bookings-actions`, mismo caso que appointment: frente aparte, SIGUE ROTA",
        "app/api/bookings/services/route.ts":
            "va con sus dos hermanas de /api/bookings, que no se abre todavía: SIGUE ROTA",
    };
    for (const [ruta, motivo] of Object.entries(seQuedanFuera)) {
        assert.ok(motivo.trim().length > 20, `la exclusión de ${ruta} no explica por qué`);
    }

    const sinPuerta = delBackend.filter((f) => {
        if (f in seQuedanFuera) return false;
        const url = "/" + f.replace(/^app\//, "").replace(/\/route\.ts$/, "");
        return !prefijos.some((p) => url.startsWith(p));
    });
    assert.deepEqual(
        sinPuerta,
        [],
        `estas rutas las llama el backend y el middleware las manda al login (307 -> /login, y fetch lo sigue, así que el backend ve un 200 y cree que funcionó):\n  ${sinPuerta.join("\n  ")}`,
    );
});
