 ...
 * panel de conversación, menos el margen interior, y crece hacia la izquierda.
...
function alFilo(g, left, right, ventana, caja) {
    ...
    const filo = Math.min(caja.right - MARGEN_INTERIOR_DE_LA_CONVERSACION, ventana - MARGEN_DE_LA_VENTANA);
    assert.equal(right, filo, `a ${ventana} el filo derecho tiene que ser el de la conversación menos su margen (${right} ≠ ${filo})`);
...
test("el margen interior es el `pr-4` de la fila de Macros y Acciones", () => {
    // Así el borde derecho de los menús cae en el píxel en que acaba Acciones.
    const texto = fs.readFileSync(join(AQUI, "../../app/(root)/chats/_components/ChatHeader.tsx"), "utf8");
    const derecho = /const MARGEN_DERECHO_DE_LA_CABECERA = 'pr-(\d+)'/.exec(texto);
    assert.ok(derecho, "falta MARGEN_DERECHO_DE_LA_CABECERA en ChatHeader");
    assert.equal(Number(derecho[1]) * 4, MARGEN_INTERIOR_DE_LA_CONVERSACION);
});
...
test("cabecera estrecha (ficha abierta): los cinco comparten el filo, no el de su botón", () => {
    ...
    assert.deepEqual([...derechos], [650], "el filo es 666 − 16, el mismo píxel en que acaba Acciones");
});
...
test("cabecera: un Macros ESCONDIDO (el de la fila del móvil) no es una medida", () => {
    ...
    assert.equal(conMacros.estilo.width, "184px", "con el Macros visible, la fila de Macros y Acciones");
});
