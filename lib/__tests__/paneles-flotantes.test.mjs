...
/**
 * Un panel de la cabecera CUELGA DE SU BOTÓN, por el lado en el que está el
 * botón: a la derecha, filo derecho con filo derecho (crece a la izquierda); a
 * la izquierda, filo izquierdo con filo izquierdo.
 */
function colgado(g, d, left, right, ventana) {
    if (g.align === "end") {
        assert.equal(right, Math.round(d.right), `a ${ventana} tiene que colgar del filo derecho de SU botón`);
    } else {
        assert.equal(g.align, "start");
        assert.equal(left, Math.round(d.left), `a ${ventana} tiene que colgar del filo izquierdo de SU botón`);
    }
}
...
test("cabecera: los seis nacen a la MISMA altura y cuelgan de SU botón", () => {
    for (const { ventana, columna: ancho } of ANCHURAS) {
        ...
        for (const d of disparadores) {
            ...
            const tope = Number(g.estilo.maxWidth.replace("px", ""));
            const { left, right } = filos(g, d, tope);
            colgado(g, d, left, right, ventana);
...
test("cabecera: con el filo de Macros, miden LO MISMO si caben, y cuelgan de su botón", () => {
    for (const { ventana, columna: ancho } of ANCHURAS) {
        ...
        const anchos = new Set();
        for (const d of disparadores) {
            const g = cabecera(caja, d, "menu", macros);

            const mide = Number(g.estilo.width.replace("px", ""));
            const { left, right } = filos(g, d, mide);
            colgado(g, d, left, right, ventana);
            assert.ok(left >= caja.left, `a ${ventana} invade la columna (${left} < ${caja.left})`);
            ...
            // el hueco.
            const sitio = g.align === "end" ? d.right - caja.left : caja.right - d.left;
            if (sitio - MARGEN_DE_LA_VENTANA >= caja.right - macros) {
...
test("cabecera: el ancho no baja del mínimo ni se pasa de la conversación", () => {
    ...
    // Y una conversación estrechísima: entonces manda ELLA —lo que hay a la
    // izquierda del botón—, no el mínimo.
    const angosta = { left: 1300, right: 1440, bottom: 148 };
    const g = cabecera(angosta, disparador, "menu", angosta.right - 20);
    const ancho = Number(g.estilo.width.replace("px", ""));
    assert.equal(ancho, disparador.right - angosta.left - MARGEN_DE_LA_VENTANA);
    assert.ok(ancho < ANCHO_MINIMO_DE_LA_CABECERA, "una caja estrecha tiene que ganarle al mínimo");
});
...
test("cabecera: a la derecha crece hacia la izquierda; a la izquierda, hacia la derecha", () => {
    const caja = { left: 480, right: 1340, bottom: 180 };
    assert.equal(cabecera(caja, { left: 1140, right: 1165, bottom: 130 }, "popover", 1140).align, "end");
    assert.equal(cabecera(caja, { left: 492, right: 520, bottom: 130 }, "popover", 1140).align, "start");
});
