 ...
 * borde de una cabecera estrecha— y entonces se corre a la derecha lo justo
 * para quedar dentro, con su borde izquierdo en el margen.
 */
function colgado(g, d, left, right, ventana, caja) {
    ...
    if (g.alignOffset === 0) {
        ...
        assert.equal(left, Math.round(caja.left + MARGEN_DE_LA_VENTANA), "se corre lo justo para no salirse");
    }
}
...
test("cabecera: el ancho no baja del mínimo ni se pasa de la conversación", () => {
    ...
    // Y una conversación estrechísima: entonces manda ELLA —la cabecera entera
    // menos sus márgenes—, no el mínimo; y como a la izquierda del botón no
    // cabe, se corre a la derecha lo justo para quedar dentro.
    const angosta = { left: 1300, right: 1440, bottom: 148 };
    const g = cabecera(angosta, disparador, "menu", angosta.right - 20);
    const ancho = Number(g.estilo.width.replace("px", ""));
    assert.equal(ancho, angosta.right - angosta.left - 2 * MARGEN_DE_LA_VENTANA);
    assert.ok(ancho < ANCHO_MINIMO_DE_LA_CABECERA, "una caja estrecha tiene que ganarle al mínimo");
    const { left, right } = filos(g, disparador, ancho);
    assert.ok(left >= angosta.left && right <= angosta.right, `se sale de la cabecera: ${left}→${right}`);
});
