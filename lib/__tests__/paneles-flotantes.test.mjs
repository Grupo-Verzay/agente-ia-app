...
import {
    ...
    MARGEN_INTERIOR_DE_LA_CONVERSACION,
    colgadoDelIcono,
    enElHilo,
    suelto,
    deSubmenu,
...
/**
 * Todo menú de la conversación tiene su filo DERECHO en el filo derecho del
 * panel de conversación, SIN margen, y crece hacia la izquierda.
 * **Da igual qué botón lo abra**: ese es el encargo entero.
 */
function alFilo(g, left, right, ventana, caja) {
    assert.equal(g.align, "end", `a ${ventana} todo menú de la conversación crece hacia la izquierda`);
    const filo = Math.min(caja.right - MARGEN_INTERIOR_DE_LA_CONVERSACION, ventana);
    assert.equal(right, filo, `a ${ventana} el filo derecho tiene que ser el de la conversación (${right} ≠ ${filo})`);
    assert.ok(left >= MARGEN_DE_LA_VENTANA, `a ${ventana} se sale por la izquierda (${left})`);
    assert.ok(left < right, "crece hacia la izquierda");
}
...
test("columnaAncha: un ancho COMÚN, colgado de SU botón y dentro de la columna, en las cuatro anchuras", () => {
    for (const { ventana, columna: ancho } of ANCHURAS) {
        const izq = CARRIL(ventana);
        const columna = { left: izq, right: izq + ancho, bottom: 200 };
        const raya = 136;
        // Uno a la izquierda (canales) y otro a la derecha (el «⌄» del final
        // de las pastillas, el de asesores).
        const izquierdo = { left: izq + 12, right: izq + 44, bottom: 96 };
        const derecho = { left: izq + ancho - 40, right: izq + ancho - 8, bottom: 128 };

        for (const [nombre, disparador] of [["izquierdo", izquierdo], ["derecho", derecho]]) {
            const g = columnaAncha(columna, disparador, raya, "menu");
            const anchoDelPanel = Number(g.estilo.width.replace("px", ""));
            const { left, right } = filos(g, disparador, anchoDelPanel);
            assert.equal(anchoDelPanel, Math.min(ANCHO_DE_LOS_FILTROS, ancho - MARGEN_DE_LA_VENTANA));
            assert.ok(left >= columna.left, `a ${ventana} el ${nombre} se sale por la izquierda (${left} < ${columna.left})`);
            assert.ok(right <= columna.right, `a ${ventana} el ${nombre} se sale por la derecha (${right} > ${columna.right})`);
            // Nace JUSTO debajo de la raya, sin hueco.
            assert.equal(disparador.bottom + g.sideOffset, raya, `a ${ventana} el ${nombre} no nace bajo la raya`);
            assert.equal(g.avoidCollisions, false, "volteado subiría sobre la cabecera");
            if (nombre === "izquierdo") {
                assert.equal(left, disparador.left, `a ${ventana} el de la izquierda alinea su filo IZQUIERDO con el del botón`);
            } else if (anchoDelPanel <= disparador.right - columna.left) {
                assert.equal(right, disparador.right, `a ${ventana} el de la derecha alinea su filo DERECHO con el del botón`);
            }
        }
    }
});

test("columnaAncha: un botón pegado a un costado corre el menú lo justo para no salirse", () => {
    const columna = { left: 48, right: 432, bottom: 200 };
    // Un botón en la mitad izquierda pero tan a la derecha que el ancho común no cabe.
    const casiMitad = { left: 220, right: 238, bottom: 96 };
    const g = columnaAncha(columna, casiMitad, 136, "menu");
    const { right } = filos(g, casiMitad, ANCHO_DE_LOS_FILTROS);
    assert.equal(right, columna.right, "se corre hasta el filo, no más");
    assert.equal(elFiloIzquierdoEnLaColumna(columna, casiMitad, ANCHO_DE_LOS_FILTROS), columna.right - ANCHO_DE_LOS_FILTROS);
});
...
test("columnaAncha: nace bajo las PASTILLAS, y los cuatro a la misma altura", () => {
    ...
    assert.equal(
        [...alturas][0],
        pastillas,
        "y esa altura es el borde de abajo de las pastillas: pegados, sin hueco",
    );
});
...
test("columnaDerecha SÍ voltea: la fila puede estar abajo del todo", () => {
    ...
    assert.equal(g.sideOffset, SEPARACION_DEL_MENU, "nace pegado a SU control, no bajo las pastillas");
});

test("columnaDerecha: los tres de una fila miden LO MISMO, acotado por la columna", () => {
    for (const { ventana, columna: ancho } of ANCHURAS) {
        const izq = CARRIL(ventana);
        const columna = { left: izq, right: izq + ancho, bottom: 900 };
        const anchos = new Set();
        for (const d of [
            ...
            const g = columnaDerecha(columna, d, "menu", 132);
            anchos.add(g.estilo.width);
            assert.equal(g.estilo.width, g.estilo.maxWidth);
        }
        assert.equal(anchos.size, 1, `a ${ventana} los de una fila tienen que medir lo mismo`);
        const mide = Number([...anchos][0].replace("px", ""));
        assert.equal(mide, Math.min(ANCHO_DE_UNA_FILA, ancho - MARGEN_DE_LA_VENTANA));
    }
});

test("columnaDerecha: volteado, el techo es el borde de abajo de las pastillas", () => {
    const g = columnaDerecha({ left: 48, right: 432, bottom: 900 }, { left: 380, right: 412, bottom: 860 }, "menu", 132);
    assert.equal(g.collisionPadding.top, 132, "volteado no sube sobre la búsqueda ni los filtros");
    assert.equal(g.collisionPadding.bottom, MARGEN_DE_LA_VENTANA);
    const sin = columnaDerecha({ left: 48, right: 432, bottom: 900 }, { left: 380, right: 412, bottom: 860 }, "menu");
    assert.equal(sin.collisionPadding.top, MARGEN_DE_LA_VENTANA, "sin pastillas medidas, el margen de siempre");
});

test("la separación es CERO y el relleno es UNO: pegados y simétricos", () => {
    assert.equal(SEPARACION_DEL_MENU, 0);
    assert.equal(RELLENO_DEL_MENU, "p-2");
});
...
test("cabecera: los seis nacen a la MISMA altura y con el MISMO filo derecho", () => {
    for (const { ventana, columna: ancho } of ANCHURAS) {
        ...
        for (const d of disparadores) {
            ...
            const tope = Number(g.estilo.maxWidth.replace("px", ""));
            const { left, right } = filos(g, d, Math.min(tope, 220));
            alFilo(g, left, right, ventana, caja);
            derechos.add(right);
        }

        assert.equal(nacen.size, 1, `a ${ventana} los seis tienen que nacer a la misma altura; salieron ${[...nacen]}`);
        assert.equal([...nacen][0], caja.bottom, "y esa altura es el borde de abajo de la cabecera");
        assert.equal(derechos.size, 1, `a ${ventana} los seis comparten filo derecho; salieron ${[...derechos]}`);
    }
});

test("cabecera: con el filo de Macros, miden LO MISMO y comparten el filo", () => {
    for (const { ventana, columna: ancho } of ANCHURAS) {
        const izq = ventana === 390 ? 0 : CARRIL(ventana) + ancho;
        const caja = { left: izq, right: ventana, bottom: 148 };
        ...
        const disparadores = [
            ...
            { left: ventana - 92, right: ventana - 16, bottom: 140 },
        ];
        ...
        for (const d of disparadores) {
            ...
            alFilo(g, left, right, ventana, caja);
            anchos.add(g.estilo.width);
            // El borde izquierdo es el de Macros: la fila de Macros y Acciones.
            assert.equal(left, macros, `a ${ventana} el menú empieza donde empieza Macros`);
        }
        assert.equal(anchos.size, 1, `a ${ventana} tienen que medir lo mismo; salieron ${[...anchos]}`);
        assert.equal(
            [...anchos][0],
            `${caja.right - MARGEN_INTERIOR_DE_LA_CONVERSACION - macros}px`,
            `a ${ventana} el ancho va del filo de Macros al filo compartido`,
        );
    }
});

test("los menús van PEGADOS al filo del recuadro: sin margen interior", () => {
    // Fueron 16 px y los menús flotaban dentro del recuadro de la
    // conversación. Lo pedido es el borde, sin separación.
    assert.equal(MARGEN_INTERIOR_DE_LA_CONVERSACION, 0);
});

test("cabecera: el ancho no baja del mínimo, y el borde izquierdo no se sale de la ventana", () => {
    ...
    // Un móvil con un ancho enorme pedido: se acota a lo que cabe a la
    // izquierda del filo, y el filo no se mueve.
    const movil = { left: 0, right: 390, bottom: 140 };
    const d = { left: 40, right: 68, bottom: 120 };
    const g = cabecera(movil, d, "popover", 1, 390);
    const w = Number(g.estilo.width.replace("px", ""));
    assert.equal(w, 390 - MARGEN_INTERIOR_DE_LA_CONVERSACION - MARGEN_DE_LA_VENTANA);
    const f = filos(g, d, w);
    alFilo(g, f.left, f.right, 390, movil);
    assert.equal(f.left, MARGEN_DE_LA_VENTANA);
});

/**
 * La captura del 23-09: con la ficha de contacto abierta la conversación es
 * estrecha, y cada menú colgaba de su botón. Abrir Cita, Registros y Acciones
 * uno tras otro movía el borde derecho de sitio. Ahora los cinco lo comparten.
 */
test("cabecera estrecha (ficha abierta): los cinco comparten el filo, no el de su botón", () => {
    ...
    for (const [nombre, d] of Object.entries(botones)) {
        ...
        alFilo(g, left, right, 1024, caja);
        derechos.add(right);
    }
    assert.deepEqual([...derechos], [666], "el filo es el borde del recuadro de la conversación, sin margen");
});
...
test("cabecera: un Macros ESCONDIDO (el de la fila del móvil) no es una medida", () => {
    ...
    const acciones = { left: 1236, right: 1324, bottom: 172 };
    ...
    assert.equal(conMacros.estilo.width, "200px", "con el Macros visible, de Macros al filo del recuadro");
});

test("el menú de llamar comparte el MISMO filo que los demás", () => {
    const caja = { left: 432, right: 1440, bottom: 148 };
    for (const d of [{ left: 900, right: 928, bottom: 96 }, { left: 440, right: 468, bottom: 96 }]) {
        const g = colgadoDelIcono(caja, d, "menu", 1440);
        assert.equal(g.estilo.width, undefined, "mide lo que ocupan sus dos entradas");
        const { left, right } = filos(g, d, 150);
        alFilo(g, left, right, 1440, caja);
        const otro = cabecera(caja, { left: 1300, right: 1332, bottom: 96 }, "menu", 1234, 1440);
        assert.equal(right, filos(otro, { left: 1300, right: 1332 }, 150).right, "el mismo filo que un menú de Macros");
    }
});
...
test("campana: el filo derecho es el de la BARRA, como Acciones el de su recuadro", () => {
    for (const { ventana } of ANCHURAS) {
        const barra = { left: 0, right: ventana, bottom: 64 };
        // El botón acaba a 12 px del borde por el `pr-3` de la barra: colgando
        // de él, el panel se quedaba ahí.
        const boton = { left: ventana - 48, right: ventana - 12, bottom: 50 };
        const g = bajoLaBarraDeArriba(barra, boton, ventana, "menu");
        const tope = Number(g.estilo.maxWidth.replace("px", ""));
        const { left, right } = filos(g, boton, tope);
        assert.equal(right, ventana, `a ${ventana} no acaba en el filo de la barra (${right})`);
        assert.ok(left >= MARGEN_DE_LA_VENTANA, `a ${ventana} se sale por la izquierda (${left})`);
    }
});
...
test("los cuatro acotan el alto con la variable de Radix, no con `vh` a secas", () => {
    ...
    for (const [nombre, g, primitiva] of todos) {
        ...
        const pad = typeof g.collisionPadding === "number" ? g.collisionPadding : g.collisionPadding.bottom;
        assert.equal(pad, MARGEN_DE_LA_VENTANA, `${nombre}: sin margen el panel se pega al borde`);
    }
});
...
// ─────────────────────────────────────────────────────────────────────────────
// La regla común: lo que se abre desde un MENSAJE, los sueltos y los submenús
// ─────────────────────────────────────────────────────────────────────────────

test("enElHilo: prefiere ARRIBA y VOLTEA si no cabe (avoidCollisions)", () => {
    const hilo = { left: 432, right: 1440, bottom: 780 };
    for (const lado of ["start", "end"]) {
        const g = enElHilo(hilo, lado, "menu");
        assert.equal(g.side, "top");
        assert.equal(g.align, lado, "se alinea con el lado del mensaje");
        assert.equal(g.avoidCollisions, true, "sin esto no voltea y se queda fuera del hilo");
        assert.equal(g.collisionPadding, MARGEN_DE_LA_VENTANA);
        assert.equal(g.alignOffset, 0, "no se desplaza a mano: el lado lo elige Floating UI");
    }
});

test("enElHilo: nunca más ancho que el hilo ni más alto que el hueco", () => {
    for (const [l, r] of [[432, 1440], [400, 1024], [0, 390], [0, 200]]) {
        const g = enElHilo({ left: l, right: r, bottom: 700 }, "end", "menu");
        const ancho = Number(g.estilo.maxWidth.replace("px", ""));
        assert.ok(ancho <= r - l - MARGEN_DE_LA_VENTANA * 2 + 1, `${l}→${r}: ${ancho}`);
        assert.ok(ancho >= 0);
        assert.match(g.estilo.maxHeight, /--radix-dropdown-menu-content-available-height/);
        assert.ok(g.estilo.maxHeight.includes(TOPE_DE_FILA));
    }
});

test("suelto: voltea, deja margen con el borde y acota el alto al hueco", () => {
    for (const prim of ["menu", "popover"]) {
        for (const side of ["top", "bottom"]) {
            const g = suelto(prim, side, "end");
            assert.equal(g.side, side);
            assert.equal(g.avoidCollisions, true);
            assert.equal(g.collisionPadding, MARGEN_DE_LA_VENTANA);
            assert.ok(g.style.maxHeight.includes(alturaDisponible(prim)));
            assert.ok(g.style.maxHeight.includes(TOPE_FIJADO));
        }
    }
});

test("deSubmenu: los submenús voltean y dejan margen, con el tope de siempre", () => {
    const g = deSubmenu();
    assert.equal(g.avoidCollisions, true);
    assert.equal(g.collisionPadding, MARGEN_DE_LA_VENTANA);
    assert.equal(g.style.maxHeight, `min(${TOPE_DE_FILA}, ${alturaDisponible("menu")})`);
});

test("ningún flotante de Chats se abre con un `div` absoluto: todos van en portal", () => {
    // El fallo de la barra de reacciones: un `absolute bottom-8` DENTRO del hilo
    // que se desplaza. Lo que se abre flotando va por Radix (portal + voltear);
    // si vuelve un `absolute bottom-*` en uno de estos, vuelve el recorte.
    const RAIZ = join(AQUI, "..", "..");
    const leer = (f) =>
        ROTO
            ? execFileSync("git", ["show", `${process.env.ANTES_REF ?? "093f071"}:${f}`], { encoding: "utf8", cwd: RAIZ })
            : fs.readFileSync(join(RAIZ, f), "utf8");
    const FICHEROS = {
        "app/(root)/chats/_components/MessageContextMenu.tsx": /absolute bottom-8/,
        "app/(root)/chats/_components/attachment-menu.tsx": /absolute bottom-12/,
        "app/(root)/chats/_components/ChatInputBar.tsx": /absolute bottom-9 left-0 z-50/,
    };
    // Sin comentarios: el propio arreglo explica al lado qué había, y buscar
    // sobre el texto crudo haría que esa explicación tumbara al banco.
    const sinComentarios = (t) => t.replace(/\/\*[\s\S]*?\*\//g, "").replace(/^\s*\/\/.*$/gm, "");
    const conAbsoluto = Object.entries(FICHEROS)
        .filter(([f, patron]) => patron.test(sinComentarios(leer(f))))
        .map(([f]) => f);
    if (ROTO) {
        assert.equal(conAbsoluto.length, 3, `el «antes» tenía los tres a mano: ${conAbsoluto}`);
        return;
    }
    assert.deepEqual(conAbsoluto, [], "vuelven a abrirse con un `div` absoluto");
    const menu = leer("app/(root)/chats/_components/MessageContextMenu.tsx");
    assert.match(menu, /usePanelFlotante\('enElHilo'/, "el menú del mensaje pasa por la regla común");
});
