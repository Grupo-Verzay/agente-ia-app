...
test("cabecera: con el filo de Macros, los seis miden LO MISMO y es del filo al filo", () => {
    for (const { ventana, columna: ancho } of ANCHURAS) {
        ...
        for (const d of disparadores) {
            ...
            assert.equal(right, caja.right, `a ${ventana} tiene que pegarse al filo derecho`);
            assert.ok(left >= caja.left, `a ${ventana} invade la columna (${left} < ${caja.left})`);
        }

        assert.equal(anchos.size, 1, `a ${ventana} los cuatro tienen que medir lo mismo; salieron ${[...anchos]}`);
        assert.equal(
            [...anchos][0],
...
test("cabecera: el ancho no baja del mínimo ni se pasa de la conversación", () => {
    ...
    // Y una conversación estrechísima: entonces manda ELLA, no el mínimo.
    const angosta = { left: 1300, right: 1440, bottom: 148 };
    const g = cabecera(angosta, disparador, "menu", angosta.right - 20);
    const ancho = Number(g.estilo.width.replace("px", ""));
    assert.equal(ancho, angosta.right - angosta.left - MARGEN_DE_LA_VENTANA);
