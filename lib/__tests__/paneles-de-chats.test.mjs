 ...
 * cabecera con el MISMO filo derecho: el de la conversación, sin margen.
 *
 ...
 *    cabecera entera.
 * 3. Y el filo derecho: cada menú colgaba de su botón (#905) y al abrir uno
 *    tras otro el borde derecho saltaba. Ahora lo comparten todos.
 *
...
test("la ficha de Contacto entra en la exclusión: es un PanelLateral con su id", () => {
    const ficha = leer("app/(root)/chats/_components/ContactInfoPanel.tsx");
    const entra = /<PanelLateral[\s\S]*?PANEL_DE_LA_FICHA/.test(ficha);
    if (ROTO) {
        assert.equal(entra, false, "el «antes» no tenía la ficha en la exclusión");
        return;
    }
    assert.ok(entra, "ContactInfoPanel tiene que pintarse en un <PanelLateral id={PANEL_DE_LA_FICHA}>");
});
...
for (const ventana of [1440, 1280, 1024]) {
    test(`a ${ventana}: la ficha y la tarea no se quedan abiertas a la vez, y salen por la derecha`, async (t) => {
        ...
        try {
            ...
            assert.equal(reservada, "abierto", "la ficha es un panel lateral más: reserva la franja como los demás");
        } finally {
            await cerrar();
        }
    });
    ...
    for (const [id, nombre] of [["acciones", "Acciones"], ["registros", "Registros del lead"]]) {
        test(`a ${ventana}: el panel de ${nombre} tiene el filo de la conversación y crece hacia la izquierda`, async (t) => {
            if (faltaNavegador(t)) return;
            const { page, cerrar } = await abrirNavegador(ventana);
            try {
                ...
                const filo = cabecera.right;
                assert.ok(Math.abs(panel.right - filo) <= 1, `su filo derecho es el de la conversación, sin margen (${panel.right} vs ${filo})`);
                ...
                const esperado = Math.max(176, filo - macros.left);
                assert.ok(
