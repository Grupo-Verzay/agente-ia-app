 ...
 *
 * Con `MODO=roto` se carga el `lib/insignia-del-favicon.ts` del #838 tal cual,
 * sacado de git en un commit pinchado. Aquel apartaba los iconos **sacándolos
 * del `<head>`**, y esos `<link>` son de React: al navegar, React 19 los
 * desmonta con `instance.parentNode.removeChild(instance)`, `parentNode` es
 * `null` y la navegación entera revienta. Es el «No se pudo cargar la
 * pantalla» de /panel. El segundo caso de este fichero lo reproduce con el
 * MISMO React que Next lleva dentro, y en ese modo **afirma el fallo**.
 *
...
const COMPILADO = readFileSync(
  new URL(
    ROTO ? "./.compilado/antes/insignia-del-favicon.js" : "./.compilado/insignia-del-favicon.js",
    import.meta.url,
  ),
  "utf8",
);
const APP = readFileSync(new URL("./.compilado/app-de-iconos.js", import.meta.url), "utf8");

/**
 * La página de la App con React: un `<head>` SIN iconos escritos a mano —el
 * que hay lo pinta React, como hace Next con los `icons` del layout— más el
 * `apple-touch-icon`, que es el respaldo de donde se lee el icono de base.
 */
const PAGINA_REACT = `<!doctype html>
<html><head>
<meta charset="utf-8">
<title>banco de la insignia con React</title>
  ...
  return new Promise((resolve) => {
    const server = http.createServer((req, res) => {
      if (req.url.startsWith("/insignia.js")) {
        ...
        res.writeHead(200, { "content-type": "text/javascript" });
        res.end(APP);
      } else if (req.url.startsWith("/react")) {
        res.writeHead(200, { "content-type": "text/html; charset=utf-8" });
        res.end(PAGINA_REACT);
...
test("navegar con la insignia puesta no revienta el `<head>` de React", async (t) => {
  if (!chromium) return t.skip("sin playwright en este equipo");
  ...
  try {
    await page.goto(`http://127.0.0.1:${port}/react`);
    await page.waitForFunction(() => window.listo === true && window.appLista === true);
    // El `<link>` de la primera «pantalla», pintado por React y subido al
    // `<head>`. Si no estuviera, el caso no se ejercería.
    await page.waitForFunction(() => document.head.querySelector('link[href*="r=proyectos"]'));
    if (VIEJO) await page.evaluate(PONER_VIEJO);
    ...
    // Se cambia de «pantalla» tres veces, como quien salta de pestaña en /panel.
    for (const ruta of ["tickets", "diagramas", "clientes"]) {
      await page.evaluate((r) => window.irA(r), ruta);
      await page.waitForTimeout(150);
    }

    const estado = await page.evaluate(() => ({
      errores: window.errores,
      ruta: document.getElementById("ruta")?.textContent ?? null,
      iconos: Array.from(document.head.querySelectorAll('link[rel~="icon"]')).map((l) => ({
        href: l.getAttribute("href"),
        nuestro: l.hasAttribute("data-insignia"),
      })),
    }));
    ...
    if (ROTO) {
      // **El fallo, tal cual se veía.** El icono de la primera pantalla estaba
      // fuera del `<head>` y React no pudo desmontarlo.
      const todos = [...estado.errores, ...erroresDePagina].join(" | ");
      console.log("[banco] modo roto:", JSON.stringify({ todos, ruta: estado.ruta }));
      assert.match(todos, /Cannot read properties of null \(reading 'removeChild'\)/);
      assert.notEqual(estado.ruta, "clientes", "y la pantalla no llega a cambiar");
      return;
    }

    assert.deepEqual(estado.errores, [], "React no puede quejarse al navegar");
    assert.deepEqual(erroresDePagina, []);
    assert.equal(estado.ruta, "clientes", "la pantalla cambia de verdad");
    if (VIEJO) return;

    // Y la insignia SIGUE mandando: el `<link>` que React pintó en la pantalla
    // nueva se apartó solo, sin salir del `<head>`.
    assert.equal(estado.iconos.length, 1, "no puede quedar ningún otro favicon");
    assert.ok(estado.iconos[0].nuestro);
    const apartado = await page.evaluate(() => {
      const l = document.head.querySelector('link[href*="r=clientes"]');
      return l && { enElHead: l.parentNode === document.head, rel: l.getAttribute("rel") };
    });
    assert.ok(apartado, "el icono de React sigue en el `<head>`, no se sacó");
    assert.equal(apartado.enElHead, true);
    assert.notEqual(apartado.rel, "icon", "pero apartado, sin competir con el nuestro");

    // Sin pendientes vuelve el de React, tal cual, y se puede seguir navegando.
    await page.evaluate(() => window.I.quitarElIcono());
    await page.evaluate(() => window.irA("tickets"));
    await page.waitForTimeout(150);
    const final = await page.evaluate(() => ({
      errores: window.errores,
      ruta: document.getElementById("ruta")?.textContent,
      iconos: Array.from(document.head.querySelectorAll('link[rel~="icon"]')).map((l) =>
        l.getAttribute("href"),
      ),
    }));
    assert.deepEqual(final.errores, []);
    assert.equal(final.ruta, "tickets");
    assert.deepEqual(final.iconos, ["/favicon-48.png?r=tickets"], "el icono de la pantalla, solo");
  } finally {
    await navegador.close();
    server.close();
  }
