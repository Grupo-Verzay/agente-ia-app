...
test("el número va a la izquierda y en azul, como en Leads", async (t) => {
    ...
    for (const ancho of ANCHURAS) {
        ...
        try {
            ...
            // El nombre ya no cuelga del número: va en su propia columna,
            // como en Leads. Lo mide `llamadas-como-leads.test.mjs`.
            assert.equal(
                m.colorDelNumero,
                m.azulDeLeads,
...
test("dentro de Llamadas no se pinta la fila de pestañas del CRM", () => {
    ...
    // Pinchado al mismo «antes» que el resto del banco: `origin/main` pasó a
    // ser el «ahora» el día que este cambio se fusionó.
    const antes = process.env.ANTES_REF || "8471ae191e3adc9cda2bf73a6fedc45c3e971b24";
    const src = ROTO
