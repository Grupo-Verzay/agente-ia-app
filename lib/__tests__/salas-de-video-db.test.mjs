 ...
 *
 * Cómo se corre, con la base de usar y tirar y el paquete recompilado:
 *
 *     scripts/banco-salas.sh
 */
...
test("revocar cierra el enlace Y echa a quien estaba dentro", async () => {
    ...
    //
    // **Quien puede revocar NO se decide aquí**: `revocarLaSala` recibe la sala
    // y nada más, y la puerta —`puedeAdministrarLaSala`— vive en la acción. Este
    // banco llegó a afirmar lo contrario, pasándole un segundo argumento que la
    // función nunca ha tenido, así que estaba en rojo sin que la afirmación
    // dijera nada de verdad.
    const sala = await unaSala();
    await entrarConCuenta({ salaId: sala.id, personaId: "ana", nombre: "Ana" });
    assert.equal(await revocarLaSala(sala.id), true);
...
// ── La familia: /reuniones cruza cuentas ─────────────────────────────────────

test("las reuniones vivas se listan por FAMILIA, no por una sola cuenta", async () => {
    // La familia de la casa: la madre y dos hijas vinculadas. `/reuniones` lista
    // las salas de las tres para que la madre no tenga que cambiar de cuenta.
    const MADRE = "fam-madre-" + unCodigoDeSala();
    const HIJA_A = "fam-hija-a-" + unCodigoDeSala();
    const HIJA_B = "fam-hija-b-" + unCodigoDeSala();
    const AJENA = "fam-ajena-" + unCodigoDeSala();
    const enUnRato = new Date(Date.now() + 3600e3);

    const sala = (cuentaId, canalId = null) =>
        crearLaSala({
            cuentaId,
            canalId,
            anfitrionId: "quien",
            anfitrionNombre: "Quien",
            titulo: null,
            expiraEn: enUnRato,
        });

    const [sMadre, sHijaA, sHijaB] = await Promise.all([sala(MADRE), sala(HIJA_A), sala(HIJA_B)]);
    await sala(AJENA); // fuera de la familia
    // Una sala DE CANAL de la madre no entra en la lista de reuniones: la puerta
    // es `canalId IS NULL`, no la cuenta.
    await sala(MADRE, "canal-x");

    // La madre ve las salas de las TRES cuentas de su familia, y ninguna ajena
    // ni la de canal.
    const familia = [MADRE, HIJA_A, HIJA_B];
    const vistasPorLaMadre = await lasSalasVivasDeLaFamilia(familia);
    const ids = new Set(vistasPorLaMadre.map((s) => s.id));
    assert.equal(ids.has(sMadre.id), true, "la madre ve su sala");
    assert.equal(ids.has(sHijaA.id), true, "la madre ve la de una hija");
    assert.equal(ids.has(sHijaB.id), true, "la madre ve la de la otra hija");
    assert.equal(
        vistasPorLaMadre.filter((s) => familia.includes(s.cuentaId)).length,
        vistasPorLaMadre.length,
        "no se cuela ninguna sala de fuera de la lista pedida",
    );
    // Cada sala trae SU cuenta dueña, que es lo que la pantalla pinta.
    assert.equal(vistasPorLaMadre.find((s) => s.id === sHijaA.id).cuentaId, HIJA_A);

    // Una hija sola ve solo lo suyo: la lista la acota el servidor a lo que
    // alcanza, y `= ANY` no deja pasar más por más ids que se manden.
    const vistasPorLaHija = await lasSalasVivasDeLaFamilia([HIJA_A]);
    assert.deepEqual(
        vistasPorLaHija.map((s) => s.id).sort(),
        [sHijaA.id].sort(),
        "la hija ve solo la suya",
    );

    // Alguien de fuera de la familia (lista vacía) no ve ninguna.
    assert.deepEqual(await lasSalasVivasDeLaFamilia([]), [], "sin familia, ninguna");
});
