 ...
 * el alcance sale de FILAS —`linked_accounts`— y no de un parámetro. Lo único
 * fingido es `currentUser()` y `revalidatePath`.
 *
 ...
 *  - Ni un reseller a su cartera ni una cuenta ajena a la familia alcanzan a
 *    nadie: lo único que hace hija a una cuenta es `linked_accounts`. El
 *    alcance a secas tiene su propio banco (`embudos-alcance-db.test.mjs`).
...
test("un reseller NO alcanza a su cliente: la cartera no es un vínculo", async () => {
    await sembrar();
    if (ROTO) return;
    como.reseller();
    const t = await pedir(() => tableroDelEmbudoAction(null, CLIENTE_R, null), "el reseller pide su cliente");
    /*
     * Esto era al revés hasta el #948, y era el fallo: el alcance sumaba
     * `clientesDeLaCuenta`, así que un reseller ofrecía su cartera y una cuenta
     * de la casa **todas las cuentas cliente de la plataforma**. Administrar o
     * facturar a un cliente no lo mete en la estructura de una cuenta: eso lo
     * contesta `/panel/clientes`, no el selector de un tablero.
     */
    assert.equal(t.cuentaId, RESELLER, "su cliente no cuelga de él por `linked_accounts`");
    assert.equal(t.puedeElegirCuenta, false, "y sin hijas no se pinta selector ninguno");
    assert.deepEqual(t.cuentas, []);
});
