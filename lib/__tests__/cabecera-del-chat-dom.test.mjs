 ...
 *    conversación (sin margen) y crece hacia la izquierda**, esté donde esté
...
test("«Más» lleva a las pestañas plegadas, y la elegida sigue a la vista", async (t) => {
    ...
    try {
        ...
        assert.ok(Math.abs(menu.right - cabecera.right) <= 1, `filo de la conversación, sin margen (${menu.right} vs ${cabecera.right})`);
