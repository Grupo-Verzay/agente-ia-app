 ...
 * 2. **El menú de la cita agendada tiene su filo derecho en el de la
 *    conversación (menos 16 px) y crece hacia la izquierda**, esté donde esté
 *    su botón. El «antes» elegía el lado por la mitad de la cabecera y crecía
...
test("«Más» lleva a las pestañas plegadas, y la elegida sigue a la vista", async (t) => {
    ...
    try {
        ...
        // El menú comparte el filo de la conversación, como todos.
        const menu = await caja(page, "[role=menu]");
        const cabecera = await caja(page, "[data-cabecera-de-chat]");
        assert.ok(menu.left >= 7, "el menú no se sale de la pantalla");
        assert.ok(Math.abs(menu.right - (cabecera.right - 16)) <= 1, `filo de la conversación menos 16 (${menu.right} vs ${cabecera.right - 16})`);
...
// ─────────────────────────────────────────────────────────────────────────────
// 2. La cita agendada comparte el filo de la conversación
// ─────────────────────────────────────────────────────────────────────────────

for (const izquierda of [false, true]) {
    test(`la cita agendada ${izquierda ? "en la mitad IZQUIERDA" : "a la derecha"}: filo derecho en el de la conversación, crece hacia la izquierda`, async (t) => {
        if (faltaNavegador(t)) return;
        const { page, cerrar } = await abrir();
        try {
            ...
            if (ROTO) return;
            const filo = cabecera.right - 16;
            assert.ok(Math.abs(panel.right - filo) <= 1, `su filo derecho es el de la conversación menos 16 (${panel.right} vs ${filo})`);
            assert.ok(panel.left < panel.right, "crece hacia la izquierda");
            assert.ok(panel.left >= 7, "sin salirse de la pantalla");
