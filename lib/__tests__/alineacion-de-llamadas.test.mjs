 ...
 *   3. El nombre, la fecha y el detalle en negrilla y en el color del texto,
 *      no en gris.
 *
...
for (const ancho of [1440, 1280, 1024]) {
    test(`a ${ancho}: encabezados centrados, contenido alineado y negrilla como en Leads`, async (t) => {
        ...
        try {
            ...
            // 3. Negrilla y en el color del texto
            for (const k of EN_NEGRILLA) {
                const e = m.negrilla[k];
                assert.ok(e, `no se encontró el texto de ${k}`);
                assert.ok(e.grosor >= 500, `${k} no va en negrilla (${e.grosor})`);
