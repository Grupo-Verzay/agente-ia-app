...
for (const medida of ANCHURAS) {
    test(`a ${medida.ventana}: los cinco paneles anchos miden la columna y nacen bajo las pastillas`, async (t) => {
        ...
        try {
            ...
            if (ROTO) {
                ...
                const miden = medidos.filter((m) => Math.abs(m.panel.ancho - m.contenedor.ancho) <= 1);
                t.diagnostic(
                    `origin/main a ${medida.ventana}: se salían ${seSalen.length} de ${medidos.length}, ` +
                        `tapaban las pastillas ${tapan.length}, medían la columna ${miden.length}, ` +
                        `y nacían a ${[...nacen].sort((a, b) => a - b).join("/")}`,
                );
                assert.ok(tapan.length > 0, "el «antes» tenía que tapar la fila de pastillas");
                assert.equal(miden.length, 0, "y ninguno medía la columna entera");
            ...
            for (const { id, panel, contenedor, pastillas, seDesplaza } of medidos) {
                ...
                assert.ok(
                    Math.abs(panel.ancho - contenedor.ancho) <= 1,
                    `${id} tiene que medir la columna entera (${panel.ancho} vs ${contenedor.ancho})`,
                );
                assert.ok(
