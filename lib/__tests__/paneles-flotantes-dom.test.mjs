...
for (const medida of ANCHURAS) {
    test(`a ${medida.ventana}: los seis de la cabecera nacen a la misma altura y con el MISMO filo derecho`, async (t) => {
        if (faltaNavegador(t)) return;
        if (medida.movil) {
        ...
        try {
            ...
            for (const id of DE_LA_CABECERA) {
                ...
                assert.ok(
                    Math.abs(panel.right - (contenedor.right - 16)) <= 1,
                    `${id}: su filo derecho es el de la conversación menos 16 (${panel.right} vs ${contenedor.right - 16})`,
                );
                nacen.add(panel.top);
                derechos.add(Math.round(panel.right));
            }
            ...
            assert.equal(derechos.size, 1, `los seis comparten filo derecho; salieron ${[...derechos]}`);
        } finally {
            await cerrar();
        }
    });
