...
test("2 · el diálogo CON resumen y transcripción los trae COMPLETOS, y sin la síntesis del lead", { skip: saltar }, async () => {
    ...
    // Con los turnos marcados, «Cliente:» se pinta como su icono (ver
    // lib/turnos-de-la-transcripcion.ts), así que se busca la frase.
    assert.ok(d.texto.includes("Llámame el jueves por la tarde."), "la transcripción no va completa");
...
test("3 · el reproductor enseña la duración DESDE QUE ABRE, sin pulsar play", { skip: saltar }, async () => {
    ...
    // Desde #921 la grabación es la nota de voz de Chats (su `<audio>` nativo
    // con `preload="metadata"`): que el total salga sin pulsar play con un
    // audio de verdad lo prueba `scripts/banco-detalle-de-llamada.sh`. Aquí la
    // grabación no existe a propósito, así que solo se afirma la forma.
    assert.equal(d.tiempo, null, "vuelve a haber un reproductor propio en vez de la nota de voz");
