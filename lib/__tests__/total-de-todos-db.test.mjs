...
// ── Lo que la PANTALLA hace al resolver y reabrir ──────────────────────────
//
// El test de arriba relee las marcas de la base, y por eso estaba verde con el
// fallo vivo en produccion: la cuenta era buena, pero la pantalla no se
// enteraba. Resolver no pintaba nada en memoria y reabrir solo limpiaba la
// llave GLOBAL, mientras la lista lee la de su linea (`linea::numero`). Aqui
// se trabaja sobre el mapa de sesiones del navegador, con sus dos llaves.

test("la pantalla pinta resolver y reabrir en TODAS las llaves, y el numero las sigue", () => {
  const ahora = Date.now();
  const sesionesEnMemoria = {};
  for (let i = 1; i <= 4; i++) {
    const jid = `57300000${i}@s.whatsapp.net`;
    const s = { id: 900 + i, resolvedAt: null };
    sesionesEnMemoria[jid] = s;
    sesionesEnMemoria[`L::${jid}`] = s;
  }
  // Lo mismo que hace `channelCounts`: la sesion de SU linea.
  const filas = (mapa) =>
    Object.keys(mapa)
      ...
      .map((k) => ({
        ...
        linea: "L",
        conSesion: true,
        activa: !estaResuelta(ahora - 60_000, mapa[k].resolvedAt),
      }));
  const servidor = { L: 4 };
  const base = new Map();
  const numero = (mapa) => totalesDeTodos(filas(mapa), servidor, base).L;

  assert.equal(numero(sesionesEnMemoria), 4);

  // Resolver la 902.
  const resolver = (mapa) =>
    ROTO ? mapa /* antes: la accion no pintaba nada */ : conLaResolucion(mapa, [902], ahora).siguiente;
  const trasResolver = resolver(sesionesEnMemoria);
  if (ROTO) {
    assert.equal(numero(trasResolver), 4, "antes: resolver no bajaba el numero hasta el reloj de sesiones");
  } else {
    assert.equal(conLaResolucion(sesionesEnMemoria, [902], ahora).tocadas, 2, "las dos llaves");
    assert.equal(numero(trasResolver), 3);
  }

  // Reabrir la 902, partiendo de una resuelta de verdad en las dos llaves.
  const resuelta = conLaResolucion(sesionesEnMemoria, [902], ahora).siguiente;
  const base2 = new Map();
  totalesDeTodos(filas(sesionesEnMemoria), servidor, base2); // la foto de antes de resolver
  const numero2 = (mapa) => totalesDeTodos(filas(mapa), servidor, base2).L;
  assert.equal(numero2(resuelta), 3);
  const reabrir = (mapa) => {
    if (!ROTO) return conLaResolucion(mapa, [902], null).siguiente;
    // Antes: `handleSessionReopened` tocaba solo `previous[remoteJid]`.
    const jid = "573000002@s.whatsapp.net";
    return { ...mapa, [jid]: { ...mapa[jid], resolvedAt: null } };
  };
  const reabierta = reabrir(resuelta);
  if (ROTO) {
    assert.equal(numero2(reabierta), 3, "antes: reabrir no devolvia la fila de su linea");
    return;
  }
  assert.equal(numero2(reabierta), 4);

  // Un id que no esta en pantalla no toca nada, y se sabe.
  assert.equal(conLaResolucion(sesionesEnMemoria, [123456], ahora).tocadas, 0);
});
