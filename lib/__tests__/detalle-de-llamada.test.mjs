 ...
 *      Chromium (el marco, todo el ancho de su recuadro y la duración ANTES de pulsar
...
test("1 · la grabación es la nota de voz: su marco, todo su recuadro y la duración SIN pulsar play", { skip: saltar }, async () => {
    ...
    const d = await page.evaluate(() => {
        ...
        return {
            ...
            anchoDelRecuadro: nota ? nota.parentElement.parentElement.getBoundingClientRect().width : null,
    ...
    // En el detalle la nota llena su recuadro (ver banco-grabacion-del-detalle).
    assert.ok(Math.abs(d.anchoDelMarco - d.anchoDelRecuadro) < 1, "la nota no llena su recuadro");
