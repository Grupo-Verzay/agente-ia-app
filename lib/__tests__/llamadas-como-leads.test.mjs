 ...
 *
 *   1. **Las columnas son las de Leads**: WhatsApp, Nombre, Duración, Fecha,
...
const COLUMNAS = ["WhatsApp", "Nombre", "Duración", "Fecha", "Detalle", "Resultado", "Acciones"];
...
test("3. Acciones se ve siempre con el menú lateral abierto", async (t) => {
    ...
    for (const ancho of ANCHURAS_DE_ESCRITORIO) {
        ...
        try {
            ...
            // La tabla reparte el ancho como Leads (`table-auto`): a 1024 con el
            // menú abierto sus columnas ya no caben y se desplaza, igual que
            // la de Leads. Lo que no puede pasar es que Acciones se pierda
            // (va `sticky`, comprobado arriba). En 1440 y 1280 cabe entera.
            if (ancho >= 1280) assert.equal(m.seDesplaza, false, `a ${ancho} la tabla se desplaza a lo ancho`);
