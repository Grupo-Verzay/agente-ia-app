 ...
 * **afirma el fallo**. La mitad del navegador no tiene modo roto y se dice:
 * reproducir el «antes» ahí serían dos builds, uno por versión del código.
 *
...
const leerDeMain = (f) => execFileSync("git", ["show", `origin/main:${f}`], { encoding: "utf8", cwd: RAIZ });
...
test("la franja se reserva DONDE CABE, y la ficha se superpone", () => {
    ...
    assert.ok(
        ...
        "con la ficha abierta y un panel, los tres anchos no caben: la ficha se superpone",
    );
});
...
test("abrir uno cierra el otro, y el sitio reservado NO se pierde por el camino", async (t) => {
    ...
    try {
        ...
        assert.equal(primero.fichaAbsoluta, "absolute", "con un panel abierto la ficha se superpone");
