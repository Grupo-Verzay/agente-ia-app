...
test("lo que no se reconoce cae en el de por defecto", () => {
  assert.equal(elEstadoDeEntrada(null), VENTANA_POR_DEFECTO);
  assert.equal(elEstadoDeEntrada("gigante"), VENTANA_POR_DEFECTO);
  assert.equal(elEstadoDeEntrada(7), VENTANA_POR_DEFECTO);
});

test("`completa` NO se restaura al abrir: cae en `maximizada`", () => {
  // El navegador niega la pantalla completa fuera de un gesto de la persona,
  // asi que restaurarla dejaria la reunion pintada como completa dentro de una
  // pagina que no lo esta.
  assert.equal(elEstadoDeEntrada("completa"), "maximizada");
  assert.equal(loQueSeRecuerda("completa"), "maximizada");
});

test("los demas se recuerdan tal cual", () => {
  for (const e of ["pastilla", "panel", "maximizada"]) {
    assert.equal(elEstadoDeEntrada(e), e);
    assert.equal(loQueSeRecuerda(e), e);
  }
});
...
test("la escala se para en los extremos, no da la vuelta", () => {
  ...
  // justo cuando se esta intentando verla mejor.
  assert.equal(alAmpliar("completa"), "completa");
  assert.equal(alReducir("pastilla"), "pastilla");
});

test("la escala recorre los cuatro en orden", () => {
  let e = "pastilla";
  const subiendo = [e];
  for (let i = 0; i < 3; i++) subiendo.push((e = alAmpliar(e)));
  assert.deepEqual(subiendo, [...ESTADOS_DE_LA_VENTANA]);

  let b = "completa";
  const bajando = [b];
  for (let i = 0; i < 3; i++) bajando.push((b = alReducir(b)));
...
test("solo las dos que flotan se arrastran", () => {
  assert.equal(sePuedeArrastrar("pastilla"), true);
  assert.equal(sePuedeArrastrar("panel"), true);
