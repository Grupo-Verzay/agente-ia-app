...
/* ── Un documento suelto compartido con otra CUENTA ──────────────────────── */

/**
 * El caso de produccion: el dialogo de permisos se abre tambien desde un
 * documento abierto, y entonces escribe una fila de `objetoTipo: 'documento'`.
 * La unica fila que hay en la base de produccion es exactamente esa.
 */
const soloElDocumento = (extra = {}) => [
    {
        objetoTipo: "documento",
        objetoId: "d1",
        sujetoTipo: "cuenta",
...
test("compartido de uno en uno: el administrador de la otra cuenta lo edita", () => {
    const acceso = accesoAlDocumento(deVentas, documento(), espacio(), soloElDocumento());
    assert.ok(acceso);
    assert.equal(acceso.recibido, true);
    assert.equal(acceso.puedeEditar, true);
...
test("y un AGENTE de esa cuenta lo ve y no lo toca", () => {
    // Es la MISMA condicion que `accesoAlEspacio` ya tenia en su rama de
    // recibido y que al documento se le habia quedado fuera: con una fila para
    // la CUENTA, un agente entraba a escribir en la documentacion de un
    // cliente. Participa, no manda.
    const suAgente = { id: "luis", role: "user", ownerId: "ventas", advisorRole: "agente" };
    const acceso = accesoAlDocumento(suAgente, documento(), espacio(), soloElDocumento());
    assert.ok(acceso);
    assert.equal(acceso.puedeEditar, false);
});

test("pero una fila para ESA PERSONA si le deja escribir, sea agente o no", () => {
    // La diferencia importa: compartir con la cuenta alcanza al equipo entero,
    // y compartir con alguien se lo dieron a el a proposito.
    const suAgente = { id: "luis", role: "user", ownerId: "ventas", advisorRole: "agente" };
    const aEl = soloElDocumento({ sujetoTipo: "persona", sujetoId: "luis" });
    const acceso = accesoAlDocumento(suAgente, documento(), espacio(), aEl);
    assert.ok(acceso);
    assert.equal(acceso.puedeEditar, true);
});

test("dentro de la cuenta propia la fila de la cuenta sigue valiendo", () => {
    // Es lo que hace que un espacio restringido se pueda abrir a su equipo:
    // el recorte es solo para lo RECIBIDO de otra cuenta.
    const cerrado = documento({ restringido: true, creadoPorId: "atencion" });
    const aMiCuenta = soloElDocumento({ sujetoId: "atencion" });
    const acceso = accesoAlDocumento(agente, cerrado, espacio(), aMiCuenta);
    assert.ok(acceso);
    assert.equal(acceso.recibido, false);
    assert.equal(acceso.puedeEditar, true);
});
...
/* ── Renombrar y borrar el espacio: su propia puerta ─────────────────────── */

/**
 * `puedeMandarEnElEspacio` es una pregunta APARTE de `puedeGestionar`, no un
 * ensanchamiento de aquella: `puedeGestionar` decide además crear documentos
 * dentro y repartir permisos, y meterle al creador se las daria de paso.
 */

/** Alguien del equipo sin `advisorRole`: ni administrador ni agente. */
const pedro = { id: "pedro", role: "user", ownerId: "atencion", advisorRole: null };

const mandar = (quien, creadoPorId, espacioExtra = {}, permisos = []) => {
    const e = espacio(espacioExtra);
    const acceso = accesoAlEspacio(quien, e, permisos);
    if (!acceso) return null;
    return puedeMandarEnElEspacio(quien, { creadoPorId }, acceso);
};

test("pasa quien administra la cuenta, aunque no lo creara el", () => {
    assert.equal(mandar(duena, "pedro"), true);
    assert.equal(mandar(yair, "pedro"), true);
});

test("y pasa QUIEN LO CREO aunque no administre la cuenta", () => {
    // Es la mitad que no es de adorno: `canManageWorkspace` no cubre a un
    // miembro del equipo sin `advisorRole`, y esa gente crea espacios hoy. Sin
    // ella se quedaria con un espacio suyo que no puede ni renombrar.
    assert.equal(mandar(pedro, "pedro"), true);
    // Pero solo sobre el SUYO: sobre el de al lado, no.
    assert.equal(mandar(pedro, "yair"), false);
});

test("un AGENTE no manda ni sobre el que creo el", () => {
    // Participa, no manda. Y borrar un espacio se lleva por delante la
    // documentacion de sus companeros.
    assert.equal(mandar(agente, "ana"), false);
    assert.equal(mandar(agente, "yair"), false);
});

test("en uno RECIBIDO de otra cuenta no manda nadie de esta, ni con edicion", () => {
    const deOtra = { id: "e1", cuentaId: "ventas", visibilidad: "cuenta" };
    const conEdicion = [
        {
            objetoTipo: "espacio",
            objetoId: "e1",
    ...
    const acceso = accesoAlEspacio(yair, deOtra, conEdicion);
    assert.ok(acceso, "el caso no llego a alcanzar el espacio");
    assert.equal(acceso.recibido, true);
    assert.equal(acceso.puedeEditar, true, "el caso tiene que poder ESCRIBIR para probar algo");
    // Puede escribir y aun asi no puede renombrarlo ni borrarlo.
    assert.equal(puedeMandarEnElEspacio(yair, { creadoPorId: "yair" }, acceso), false);
});

test("el creador se compara con la PERSONA, no con su cuenta", () => {
    // Con la cuenta efectiva, un espacio creado por el dueno se lo daria de
    // golpe a todo su equipo, agentes incluidos.
    assert.equal(mandar(pedro, "atencion"), false);
});
