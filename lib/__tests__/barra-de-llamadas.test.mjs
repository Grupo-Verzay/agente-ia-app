...
test("la ventana se abre y se cierra", async (t) => {
    ...
    try {
        ...
        for (const rotulo of ["Cancelar", "Llamar", "Llamar con IA"]) {
            assert.ok(
        ...
        // Y «Cancelar» la cierra.
        await page.click('[data-dialogo="llamar"] >> text=Cancelar');
        await page.waitForSelector('[data-dialogo="llamar"]', { state: "detached", timeout: 5000 });
        assert.equal(
            await page.locator('[data-dialogo="llamar"]').count(),
            0,
            "«Cancelar» tiene que cerrar la ventana",
        );
    } finally {
        await cerrar();
    }
...
test("cada botón dispara SU llamada y no la otra", async (t) => {
    ...
    try {
        ...
        assert.deepEqual(
            await page.evaluate(() => window.__llamadas),
            { llamar: 1, ia: 1 },
            "«Llamar con IA» dispara la suya y no vuelve a disparar la normal",
        );
    } finally {
        await cerrar();
    }
