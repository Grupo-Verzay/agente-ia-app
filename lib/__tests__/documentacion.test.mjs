...
/* ── El selector del diálogo de permisos ──────────────────────────────────── */

const gente = [
    { sujetoTipo: "persona", sujetoId: "u1", etiqueta: "Yair Martínez", detalle: "y@v.com" },
    { sujetoTipo: "cuenta", sujetoId: "u1", etiqueta: "Verzay | Atención", detalle: "a@v.com" },
    { sujetoTipo: "cuenta", sujetoId: "u2", etiqueta: "Distribuidora Paz", detalle: "d@p.com" },
];

test("lo ya concedido se marca por TIPO + ID, nunca por el id solo", () => {
    // Una cuenta y una persona son las dos filas de `User`, asi que el mismo id
    // puede estar concedido como cuenta y no como persona. Comparando solo por
    // id, elegir a la PERSONA saldria como «ya tiene acceso» porque su cuenta
    // lo tiene, y quien reparte creeria que ya esta hecho.
    const ofrece = loQueSeOfreceParaCompartir(
        gente,
        [{ sujetoTipo: "cuenta", sujetoId: "u1", permiso: "edicion" }],
        "",
    );
    const porLlave = new Map(ofrece.map((o) => [`${o.sujetoTipo}::${o.sujetoId}`, o.yaTiene]));
    assert.equal(porLlave.get("cuenta::u1"), "edicion");
    assert.equal(porLlave.get("persona::u1"), null);
});

test("lo ya concedido NO se esconde: se marca", () => {
    // La fila de la lista de arriba solo se puede quitar, asi que escondiendolo
    // no habria forma de pasar a alguien de lectura a edicion si no es
    // quitandolo y volviendolo a poner.
    const ofrece = loQueSeOfreceParaCompartir(
        gente,
        [{ sujetoTipo: "cuenta", sujetoId: "u2", permiso: "lectura" }],
        "",
    );
    assert.equal(ofrece.length, gente.length);
});

test("el selector de permisos busca igual que el de menciones: sin acentos", () => {
    // Es la MISMA funcion de filtrar. Con dos, quien teclea «atencion» encuentra
    // «Verzay | Atención» al mencionar y no al compartir, y eso se lee como que
    // esa cuenta no se puede compartir.
    const ofrece = loQueSeOfreceParaCompartir(gente, [], "atencion");
    assert.equal(ofrece.length, 1);
    assert.equal(ofrece[0].sujetoTipo, "cuenta");
    assert.equal(ofrece[0].sujetoId, "u1");
});
