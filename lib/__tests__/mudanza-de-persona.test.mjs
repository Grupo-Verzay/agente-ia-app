...
test("MODO=roto: la cuenta nueva SIN lista no la deja sin tope", () => {
    // El caso de verdad, y salio en una mudanza de produccion: la cuenta
    // destino no tiene ninguna fila en `_UserModules` —es de administrador, o
    // de plan personalizado— y la persona tenia tres. Vaciarla no le quita
    // tres modulos: le quita el TOPE, y pasa a ver los CATORCE que el plan de
    // su cuenta nueva permite. O sea que el recorte la ENSANCHA.
    const suyos = ["chats", "herramientas", "panel"];

    if (ROTO) {
        assert.deepEqual(recorteIngenuo(suyos, []), [], "el recorte ingenuo la deja sin tope");
        return;
    }

    // El recorte solo puede QUITAR, nunca ensanchar: se conservan los suyos.
    assert.deepEqual(losModulosQueLeQuedan(suyos, []).sort(), ["chats", "herramientas", "panel"]);
    assert.deepEqual(losModulosQueSeLeQuitan(suyos, []), []);
    assert.deepEqual(losModulosQueSeLeDan(suyos, []), []);
});

test("y los dos «cruce vacio» NO son el mismo caso", { skip: ROTO }, () => {
    // Con lista y sin nada en comun: se le dan los de ella, que es un tope.
    assert.deepEqual(losModulosQueLeQuedan(["finanzas"], ["chats"]), ["chats"]);
    // Sin lista: no hay tope que dar, asi que se conserva el suyo.
    assert.deepEqual(losModulosQueLeQuedan(["finanzas"], []), ["finanzas"]);
});
