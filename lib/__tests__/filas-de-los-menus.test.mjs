...
test("el rótulo arranca donde arranca su fila, en los dos", () => {
    for (const ancho of ANCHURAS) {
        ...
        // Y se ve igual: mismo tamaño, mismo peso y los dos en mayúscula. El de
        // Etapas ya iba así; el de Etiquetas lo pinta cmdk con los suyos.
        for (const campo of ["letra", "peso", "mayuscula"]) {
            assert.equal(
                etiquetas.rotulo[campo],
                etapas.rotulo[campo],
                `los rótulos difieren en ${campo} a ${ancho}: ${etiquetas.rotulo[campo]} vs ${etapas.rotulo[campo]}`,
            );
        }
        assert.equal(etapas.rotulo.mayuscula, "uppercase", `el rótulo no va en mayúscula (${ancho})`);
