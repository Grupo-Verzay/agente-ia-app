...
test("la línea de estado va a 12 px dentro de sus 14 de interlineado", () => {
    assert.equal(c.LETRA_DEL_ESTADO, 12);
    assert.match(c.LINEA_DEL_ESTADO, new RegExp(`text-\\[${c.LETRA_DEL_ESTADO / 16}rem\\]`));
    assert.ok(c.LETRA_DEL_ESTADO <= c.ALTO_LINEA_DEL_ESTADO);
});
