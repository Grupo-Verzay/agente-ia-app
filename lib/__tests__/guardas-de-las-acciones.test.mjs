...
const FICHEROS = [
    ...
    "actions/google-sheets-actions.ts",
...
const SIN_GUARDA = {
    ...
    getScheduleRemindersByUserId:
        "la abre /schedule/[userId], pública, y devuelve SOLO los isSchedule: el filtro es la puerta",
    ...
    sendBookingNotifications:
        "la llama el navegador de quien reserva, sin sesión; su pega y qué la cerraría están escritas en el fichero",
    autoSyncContactIfEnabled:
        "la llama /api/owner/sync-contact sin sesión; no recibe ningún dato, solo adelanta un volcado que la cuenta pidió",
