...
test("pedir a Carlos a mano en el parámetro tampoco lo trae", async (t) => {
    ...
    await conLaBase(async () => {
        ...
        // Y mandar el id de Carlos COMO CUENTA PROPIA —lo que hacen las
        // acciones que reciben `userId` del navegador— tampoco: se mira desde
        // la propia.
        const comoPropia = await lasCuentasQueConsultaElCrm(CARLOS);
        assert.ok(!comoPropia.includes(CARLOS), "la madre no entra como propia");
        assert.ok(comoPropia.includes(ATENCION));
    });
});
...
test("una hoja que manda el id de su madre como propia sigue viendo solo lo suyo", async (t) => {
    if (ROTO) return t.skip("el modo roto ya afirma su fallo");
    await conLaBase(async () => {
        ponerAQuienMira(cuenta(VENTAS));
        assert.deepEqual(await lasCuentasQueConsultaElCrm(CARLOS), [VENTAS]);
        assert.deepEqual(await lasCuentasQueConsultaElCrm(ATENCION), [VENTAS]);
        ponerAQuienMira(persona(AGENTE, "agente"));
        assert.deepEqual(await lasCuentasQueConsultaElCrm(CARLOS), [ATENCION]);
    });
});
