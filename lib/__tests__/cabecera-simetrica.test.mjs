...
function lasCompartidas() {
    ...
    return { cab: leer("CABECERA_ESCRITORIO"), f1: leer("CLASE_FILA_1"), f2: leer("CLASE_FILA_2"), control: leer("CONTROL_DE_ICONO") };
}
...
function laCabecera() {
    ...
    if (texto.includes("data-cabecera-escritorio")) {
        ...
        return {
            ...
            fila1: clases(texto, /className=\{cn\('(flex shrink-0 items-center gap-3 overflow-hidden)', CLASE_FILA_1\)\}/g) + " " + k.f1,
            grupo1: clases(texto, /className="(flex min-w-0 items-center gap-1\.5 overflow-x-auto[^"]*)"/g),
