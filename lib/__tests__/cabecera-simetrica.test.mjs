...
/**
 * El texto del componente: el del árbol de trabajo, o el de ANTES_REF cuando se
 * está afirmando el «antes».
 *
 * ANTES_REF va PINCHADO al commit anterior a #888, que es el que cuadró las dos
 * filas. Leía `origin/main`, y en cuanto #888 se fusionó el modo roto dejó de
 * reproducir nada: el «antes» caduca el día que su PR entra.
 */
const ANTES_REF = process.env.ANTES_REF_SIMETRIA || "fcddcd0";
function elComponente() {
    if (!ROTO) return fs.readFileSync(join(RAIZ, COMPONENTE), "utf8");
    return execFileSync("git", ["show", `${ANTES_REF}:${COMPONENTE}`], {
        encoding: "utf8",
        cwd: RAIZ,
    });
}
...
/** Las clases compartidas de `lib/cabeceras-de-chats.ts`, leídas del fichero. */
function lasCompartidas() {
    const lib = fs.readFileSync(join(RAIZ, "lib/cabeceras-de-chats.ts"), "utf8");
    const leer = (n) => {
        const m = new RegExp(`export const ${n} = "([^"]+)"`).exec(lib);
        assert.ok(m, `falta ${n} en lib/cabeceras-de-chats.ts`);
        return m[1];
    };
    return {
        cab: leer("CABECERA_ESCRITORIO"),
        f1: leer("CLASE_FILA_1"),
        f2: leer("CLASE_FILA_2"),
        control: leer("CONTROL_DE_ICONO"),
        hueco: leer("CLASE_HUECO_ENTRE_CONTROLES"),
    };
}

function laCabecera() {
    const texto = elComponente();
    // La forma de hoy: el margen lo pone la CABECERA (`lib/cabeceras-de-chats.ts`)
    // y la ficha va fuera de la tira de iconos que se desplaza.
    if (texto.includes("data-cabecera-escritorio")) {
        const k = lasCompartidas();
        const pestanas = fs.readFileSync(join(RAIZ, "app/(root)/chats/_components/PestanasDelChat.tsx"), "utf8");
        const relleno = /<PestanasDelChat[\s\S]*?clasePestana="(px-4 h-[78])"/.exec(texto);
        const base = /'(shrink-0 text-sm font-medium whitespace-nowrap border-b-2 transition-colors)'/.exec(pestanas);
        const hueco = /data-pestanas-del-chat className="([^"]+)"/.exec(pestanas);
        assert.ok(relleno && base && hueco, "no se encontraron las clases de las pestañas");
        return {
            nuevo: true,
            cab: clases(texto, /data-cabecera-escritorio className=\{cn\('([^']+)', CABECERA_ESCRITORIO\)\}/g) + " " + k.cab,
            fila1: clases(texto, /className=\{cn\('(flex shrink-0 items-center gap-3 overflow-x-clip)', CLASE_FILA_1\)\}/g) + " " + k.f1,
            grupo1:
                clases(
                    texto,
                    /className=\{cn\('(flex min-w-0 items-center overflow-x-auto[^']*)', CLASE_HUECO_ENTRE_CONTROLES\)\}/g,
                ) +
                " " +
                k.hueco,
            ficha: clases(texto, /className=\{cn\("(hidden md:flex items-center gap-1\.5 px-2 rounded-lg[^"]*)", CONTROL_DE_ICONO\)\}/g) + " " + k.control,
            boton: clases(texto, /className="(h-8 md:h-7 gap-1\.5 px-2\.5 text-sm)"/g),
            fila2: clases(texto, /data-fila-de-pestanas className=\{cn\('(-ml-4 flex shrink-0 items-center)', CLASE_FILA_2\)\}/g) + " " + k.f2,
            hueco2: hueco[1],
            tab: `${relleno[1]} ${base[1]}`,
            grupo2: clases(texto, /data-mandos-de-la-fila className="(flex shrink-0 items-center gap-1 pl-1)"/g),
        };
    }
    const comun = {
        fila1: clases(
    ...
    // Desde que las pestañas se pliegan en «Más» la fila de abajo son DOS
    // cajas: `PestanasDelChat` (flex-1) y los mandos (shrink-0). La pestaña se
    // lee de su componente, con el relleno que le pasa la cabecera.
    if (texto.includes("data-fila-de-pestanas")) {
        const pestanas = fs.readFileSync(join(RAIZ, "app/(root)/chats/_components/PestanasDelChat.tsx"), "utf8");
        const relleno = /<PestanasDelChat[\s\S]*?clasePestana="(px-4 py-2)"/.exec(texto);
        const base = /'(shrink-0 text-sm font-medium whitespace-nowrap border-b-2 transition-colors)'/.exec(pestanas);
        const hueco = /data-pestanas-del-chat className="([^"]+)"/.exec(pestanas);
        assert.ok(relleno && base && hueco, "no se encontraron las clases de las pestañas");
        return {
            ...comun,
            fila2: clases(texto, /data-fila-de-pestanas className="(flex items-center)"/g),
            hueco2: hueco[1],
            tab: `${relleno[1]} ${base[1]}`,
            grupo2: clases(
                texto,
                /data-mandos-de-la-fila className=\{cn\('(flex shrink-0 items-center gap-1 pl-1)', (\w+)\)\}/g,
            ),
        };
    }
    return {
...
<div id="cab" class="${c.cab ?? "hidden md:flex md:flex-col md:justify-center overflow-hidden border-b-2 border-border"}" ${c.cab ? "" : 'style="height:5.125rem"'}>
  <div class="${c.fila1}">
    ...
    <div class="${c.grupo1}">
      ...
      ${c.nuevo ? "" : `<button id="ultimo1" class="${c.ficha.replace("hidden md:flex", "flex")}">F</button>`}
    </div>
    ${c.nuevo ? `<button id="ultimo1" class="${c.ficha.replace("hidden md:flex", "flex")}">F</button>` : ""}
  </div>
  <div class="${c.fila2}">
    <div class="${c.hueco2}" ${c.hueco2 ? "" : 'style="display:contents"'}>
...
test("el margen, el alto y las filas están escritos UNA vez, y cuadran entre sí", async (t) => {
    if (ROTO) {
        t.skip("en el «antes» no había ninguna constante que comprobar");
        return;
    }
    const lib = fs.readFileSync(join(RAIZ, "lib/cabeceras-de-chats.ts"), "utf8");
    const num = (n) => Number(new RegExp(`export const ${n} = (\\d+);`).exec(lib)?.[1]);
    const margen = num("MARGEN_DE_LAS_CABECERAS");
    const f1 = num("ALTO_FILA_1");
    const f2 = num("ALTO_FILA_2");
    const hueco = num("HUECO_ENTRE_FILAS");
    const borde = num("BORDE_DE_LA_CABECERA");
    assert.equal(margen, 6, "las cabeceras van apretadas al mínimo");
    const cab = /export const CABECERA_ESCRITORIO = "md:h-\[([\d.]+)rem\] md:p-([\d.]+) md:gap-([\d.]+)"/.exec(lib);
    assert.ok(cab, "CABECERA_ESCRITORIO no tiene la forma esperada");
    assert.equal(Number(cab[1]) * 16, margen * 2 + f1 + hueco + f2 + borde, "el alto sale de las filas");
    assert.equal(Number(cab[2]) * 4, margen, "el relleno es el margen");
    assert.equal(Number(cab[3]) * 4, hueco, "el hueco entre filas");
    assert.equal(Number(/CLASE_FILA_1 = "md:h-(\d+)"/.exec(lib)[1]) * 4, f1);
    assert.equal(Number(/CLASE_FILA_2 = "md:h-(\d+)"/.exec(lib)[1]) * 4, f2);

    // Y las DOS cabeceras las usan: ninguna vuelve a escribir sus números.
    const conv = fs.readFileSync(join(RAIZ, COMPONENTE), "utf8");
    const col = fs.readFileSync(join(RAIZ, "app/(root)/chats/_components/chat-sidebar.tsx"), "utf8");
    for (const [nombre, texto] of [["ChatHeader", conv], ["chat-sidebar", col]]) {
        assert.ok(texto.includes("@/lib/cabeceras-de-chats"), `${nombre} no usa lib/cabeceras-de-chats`);
        assert.ok(!texto.includes("'5.125rem'"), `${nombre} volvió a escribir el alto a mano`);
    }
    assert.ok(!conv.includes("MARGEN_DERECHO_DE_LA_CABECERA"), "la cabecera volvió a escribir su margen a mano");
});
