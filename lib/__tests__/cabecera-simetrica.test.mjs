...
function lasCompartidas() {
    ...
    return { cab: leer("CABECERA_ESCRITORIO"), f1: leer("CLASE_FILA_1"), f2: leer("CLASE_FILA_2") };
}
...
function laCabecera() {
    ...
    if (texto.includes("data-cabecera-escritorio")) {
        ...
        const relleno = /<PestanasDelChat[\s\S]*?clasePestana="(px-4 h-8)"/.exec(texto);
...
test("el margen, el alto y las filas están escritos UNA vez, y cuadran entre sí", async (t) => {
    ...
    assert.equal(margen, 16, "el margen es el de la fila de botones de la conversación");
