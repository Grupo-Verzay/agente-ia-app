...
/* Desde que el color es libre, lo sembrado es un hex y no un índice de paleta.
   `COLORES_RAPIDOS` se lee del módulo y no se copia: con la lista escrita aquí,
   el banco probaría que coincide con lo que alguien recuerda. */
const { COLORES_RAPIDOS } = await import(join(AQUI, ".compilado", "embudos.js"));
const COLOR_ELEGIDO = "#7C3AED";
...
test("sembrar", async () => {
    ...
    await guardarEtapas(MADRE, ESTADO.ventas, [
        { id: null, nombre: "Contactado", color: COLOR_ELEGIDO },
...
test("la etapa guardada manda; la de otro embudo no se cuela y la borrada cae en la primera", async () => {
    ...
    await guardarEtapas(MADRE, ESTADO.ventas, [
        { id: ESTADO.etapasDeVentas[0].id, nombre: "Contactado", color: COLOR_ELEGIDO },
...
test("el color: el elegido si lo tiene, y si no el de su posición", async () => {
    ...
    assert.equal(primera.color, COLOR_ELEGIDO, "la sembrada con color elegido");
    assert.equal(segunda.color, null, "la sembrada sin color");

    const mapa = await comoLaBandeja([ESTADO.conAsesor]);
    assert.equal(mapa.get(ESTADO.conAsesor)?.color, COLOR_ELEGIDO, "manda el color elegido");

    como(MADRE);
    await moverTarjetaAction(ESTADO.conAsesor, segunda.id);
    const despues = await comoLaBandeja([ESTADO.conAsesor]);
    assert.equal(
        despues.get(ESTADO.conAsesor)?.color,
        COLORES_RAPIDOS[1],
        "sin color elegido manda la posición (la 2.ª)",
    );
});
