 ...
 *      el desplegable «Marcar resultado» con sus cinco opciones; con resultado,
 *      su pastilla.
...
// Los cinco del cambio de resultados («Agendó» pasó a «Link enviado»; «Buzón de voz» y
// «Número equivocado» se fueron). El roto también pinta estos: lo que ese modo
// afirma es el guion de la hija, no la lista.
const RESULTADOS = ["Interesado", "Link enviado", "Volver a llamar", "No contesta", "No interesado"];
...
test("2. Resultado: «Marcar resultado» también en la llamada de una hija; con resultado, su pastilla", async (t) => {
    ...
    try {
        ...
        // Y el desplegable de la hija ofrece las cinco, igual que el de la propia.
