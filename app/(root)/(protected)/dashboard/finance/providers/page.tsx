...
export default async function ProvidersPage({
  ...
  const listRes = await getFinanceContacts(user.id, "SUPPLIER");
  ...
  return (
    <div className="flex flex-1 min-h-0 flex-col p-4 sm:p-6">
      <MainFinanceContacts userId={user.id} kind="SUPPLIER" contacts={contacts} autoOpenCreate={autoOpenCreate} />
    </div>
  );
}
