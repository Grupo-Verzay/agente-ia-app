 ...
 *
 * Y no lleva cabecera cuando no hay carpetas: sin ninguna carpeta creada, el
 * árbol es el de siempre y un rótulo «Sueltos» sobre la lista entera sería
 * ruido que nadie pidió.
 */
export function EspaciosSueltos({
    ...
    return (
        <div
            ...
            className={cn(
                "rounded",
                // Con carpetas encima hace falta un sitio donde soltar aunque no
                // quede ningún espacio suelto: sin alto mínimo, sacar el último
                // de una carpeta sería imposible.
                hayCarpetas && "min-h-10",
                isOver && "bg-muted/60 ring-1 ring-primary/40",
            )}
        >
            {hayCarpetas && (
                <p className="px-3 py-1 text-[11px] font-medium uppercase tracking-wide text-muted-foreground">
                    {vacio ? "Sin carpeta · arrastra aquí para sacar un espacio" : "Sin carpeta"}
                </p>
            )}
            {children}
        </div>
